...
{
  page_ptr = (const uint8_t*) lookup + page_id*page_size;
...
#ifdef HELLO
//Print chip ID http://atmel.force.com/support/articles/en_US/FAQ/Reading-unique-serial-number-on-SAM-D20-SAM-D21-SAM-R21-devices
void printChipID()
{
...
#ifdef HELLO
      //A command of zyx turns on the hello rpc interface 
        case 'z':
        while (SerialUSB.available() == 0)  {}
        if ((char)SerialUSB.read()=='y')
        {
          while (SerialUSB.available() == 0)  {}
...
#ifdef HELLO
  SerialUSB.println(" b - print chip ID");
#endif
  SerialUSB.println("");
